...
function expandStartForAdjacency(turns: Turn[], start: number): number {
	...
	if (turns[start]!.role === "tool") {
		...
		while (blockStart > 0 && turns[blockStart - 1]!.role === "tool")
			blockStart--;
		if (
			blockStart > 0 &&
			turns[blockStart - 1]!.role === "assistant" &&
			(turns[blockStart - 1]!.tool_calls?.length ?? 0) > 0
		) {
			return blockStart - 1;
		}
	}
...
function expandEndForAdjacency(turns: Turn[], end: number): number {
	...
	if (
		...
		turns[end - 1]!.role === "assistant" &&
		(turns[end - 1]!.tool_calls?.length ?? 0) > 0 &&
		turns[end]!.role === "tool"
	) {
		return toolBlockEnd(turns, end);
	}
	...
	while (start < turns.length) {
		...
		if (end - start > MAX_TURNS) {
			...
			if (end < turns.length && turns[end]!.role === "tool") {
				let blockStart = end;
				while (blockStart > start && turns[blockStart - 1]!.role === "tool")
					blockStart--;
				if (
					blockStart > start &&
					turns[blockStart - 1]!.role === "assistant" &&
				...
				isToolAdjacencyBreak(turns, end)
			) {
				end--;
			}
		}
		...
		chunks.push({
			...
			meta: {
				...
				chunk_total: 0,
				overlap_from: start > 0 ? OVERLAP : null,
			},
		});
		...
		while (
			nextStart > 0 &&
			nextStart < turns.length &&
			turns[nextStart]!.role === "tool"
		) {
			let bs = nextStart;
			while (bs > 0 && turns[bs - 1]!.role === "tool") bs--;
			if (
				bs > 0 &&
				turns[bs - 1]!.role === "assistant" &&
				(turns[bs - 1]!.tool_calls?.length ?? 0) > 0 &&
				bs - 1 < nextStart
			) {
				nextStart = bs - 1;
				break;
