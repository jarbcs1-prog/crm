...
export function definedScores(record: CallRecord): Array<{ key: string; value: number }> {
  const out: Array<{ key: string; value: number }> = [];
  for (const key of CALL_SCORE_KEYS) {
    const v = record.scores?.[key];
    if (typeof v === "number" && Number.isFinite(v)) out.push({ key, value: v });
  }
  return out;
}
...
export interface JudgeResult {
  ...
  callId: string;
  score: number;
  checks: { hasTranscript: boolean; hasOutcome: boolean; hasScores: boolean; hasSummary: boolean };
}
...
export function judgeCallSession(session: Session): JudgeResult {
  ...
  const checks = {
    ...
    hasSummary: session.turns.some((t) => t.role === "assistant" && t.content.includes("Summary:")),
  };
...
export function arenaCompare(sessions: Session[], variantOf: (s: Session) => string | null): ArenaReport {
  ...
  const ranked = [...groups.keys()].sort((a, b) =>
    createHash("sha256").update(a).digest("hex") < createHash("sha256").update(b).digest("hex") ? -1 : 1,
  );
  ...
  const winner = blinded.length
    ? blinded.slice().sort((a, b) => b.meanScore - a.meanScore || b.n - a.n)[0]!.label
    : null;
  return {
    blinded: blinded.map(({ label, n, meanScore }) => ({ label, variant: label, n, meanScore })),
    reveal,
    winner,
...
export function variantByPitch(session: Session): string | null {
  const extra = (session.meta.extra ?? {}) as Record<string, unknown>;
  return typeof extra.pitchId === "string" && extra.pitchId ? extra.pitchId : null;
}
...
export function writeCallDataset(records: CallRecord[], outDir: string): CallDatasetOutputs {
  ...
  writeFileSync(join(outDir, "judge.json"), JSON.stringify(outputs.judged, null, 2), "utf-8");
  writeFileSync(join(outDir, "arena.json"), JSON.stringify(outputs.arena, null, 2), "utf-8");
  const lines = [
    "# Call dataset arena (blind)",
    "",
    ...outputs.arena.blinded.map((e) => `- ${e.label}: n=${e.n} mean=${e.meanScore.toFixed(3)}`),
    "",
    `Winner: ${outputs.arena.winner ?? "none"}`,
    "",
    "## Reveal",
    "",
    ...Object.entries(outputs.arena.reveal).map(([label, variant]) => `- ${label} = ${variant}`),
    "",
    outputs.arena.note,
    "",
  ];
...
if (isMain) {
  ...
  if (!input) {
    console.error("Usage: bun run src/export/callDataset.ts --input <calls.json> --out <dir>");
    process.exit(1);
  }
  const raw = JSON.parse(readFileSync(input, "utf-8") as string) as CallRecord[] | { calls: CallRecord[] };
  const records = Array.isArray(raw) ? raw : raw.calls;
  const outputs = writeCallDataset(records, out);
  console.log(`calls: ${records.length} in, ${outputs.sessions.length} sessions, ${outputs.skipped} skipped, ${outputs.duplicates} duplicates, ${outputs.redacted} redacted -> ${out}`);
}
