...
export const MANIFEST: ManifestFieldMapping[] = [
	{
		canonicalField: "turns[].content (user first)",
		easyDatasetColumn: "question",
		required: true,
		transform: "first user turn content, trimmed",
	},
	{
		canonicalField: "turns[].content (assistant last)",
		easyDatasetColumn: "answer",
		required: true,
		transform: "last assistant content or serialized tool_calls",
	},
	{
		canonicalField: "turns[].reasoning_content",
		easyDatasetColumn: "cot",
		required: false,
		transform: "concatenated reasoning_content joined by \\n\\n",
	},
	{
		canonicalField: "meta.title / session.id",
		easyDatasetColumn: "chunkName",
		required: false,
		transform: "meta.title ?? session.id",
	},
	{
		canonicalField: "turns[].content (all)",
		easyDatasetColumn: "chunkContent",
		required: false,
		transform: "truncated concatenation of all turn contents (max 2000 chars)",
	},
	{
		canonicalField: "meta.model",
		easyDatasetColumn: "model",
		required: false,
		transform: "meta.model ?? 'imported'",
	},
	{
		canonicalField: "meta.task_type / tags[0]",
		easyDatasetColumn: "questionLabel",
		required: false,
		transform: "meta.task_type ?? tags[0] ?? ''",
	},
	{
		canonicalField: "meta.tags",
		easyDatasetColumn: "tags",
		required: false,
		transform: "JSON.stringify(tags)",
	},
	{
		canonicalField: "confirmed",
		easyDatasetColumn: "confirmed",
		required: false,
		transform: "true (pre-validated)",
	},
	{
		canonicalField: "score",
		easyDatasetColumn: "score",
		required: false,
		transform: "1.0 default",
	},
	{
		canonicalField: "meta.extra.note",
		easyDatasetColumn: "note",
		required: false,
		transform: "meta.extra.note ?? ''",
	},
	{
		canonicalField: "meta",
		easyDatasetColumn: "other",
		required: false,
		transform: "JSON.stringify({source, session_id, chunk_index})",
	},
	{
		canonicalField: "turns[] (full)",
		easyDatasetColumn: "rawMessages (DatasetConversations)",
		required: false,
		transform: "JSON.stringify ShareGPT messages for multi-turn",
	},
	{
		canonicalField: "turns.length",
		easyDatasetColumn: "turnCount/maxTurns (DatasetConversations)",
		required: false,
		transform: "turnCount = turns.length, maxTurns = turnCount",
	},
];

function extractCot(session: Session): string {
	const parts = session.turns
		.map((t) => t.reasoning_content)
...
function extractAnswer(session: Session): string {
	...
	if (last.tool_calls?.length)
		return JSON.stringify(
			last.tool_calls.map((tc) => ({ name: tc.name, arguments: tc.arguments })),
		);
	return "";
}

function chunkContent(session: Session): string {
	const combined = session.turns
...
export function sessionToEasyDatasetRecord(
	session: Session,
): EasyDatasetRecord | null {
	...
	return {
		...
		other: JSON.stringify({
			...
			has_tool_use: session.meta.has_tool_use,
			has_reasoning: session.meta.has_reasoning,
		}),
	};
}
...
export function buildBundles(sessions: Session[]): {
	datasets: EasyDatasetImportBundle;
	conversations: EasyDatasetConversationBundle;
} {
	...
	for (const s of sessions) {
		...
		const hasMultiTurn =
			s.turns.length > 2 || s.turns.some((t) => !!t.tool_calls?.length);
		if (hasMultiTurn) {
...
export function writeBundles(sessions: Session[], outDir: string): void {
	...
	writeFileSync(
		join(outDir, "easydataset-import.json"),
		JSON.stringify(datasets, null, 2),
		"utf-8",
	);
	writeFileSync(
		join(outDir, "easydataset-conversations.json"),
...
export function writeToSqlite(
	sessions: Session[],
	dbPath: string,
	projectId: string,
): void {
	const { default: Database } = require("better-sqlite3") as unknown as {
...
if (isMain) {
	...
	if (!input) {
		console.error(
			"Usage: bun run src/export/easyDataset.ts --input <canonical.jsonl> --out <dir> [--sqlite <db.sqlite> --projectId <id>]",
		);
		process.exit(1);
	}
	if (!existsSync(input)) {
	...
	console.log(
		`Wrote ${sessions.length} sessions -> ${out}/easydataset-import.json + conversations + manifest`,
	);
	if (sqlitePath) {
