...
describe("synthesizeSession", () => {
  it("alternates assistant segments with persona replies and closes", () => {
    ...
    expect(s.turns[0]).toMatchObject({ role: "assistant", content: "Hello, thirty seconds?" });
    expect(s.turns[1]).toMatchObject({ role: "user", content: "Yes, I have a moment." });
    expect(s.turns[s.turns.length - 1]!.role).toBe("assistant");
  });

  it("falls back to defaultReply when the script runs short", () => {
    const s = synthesizeSession("demo", [...segments, { pitch: "demo", id: "s3", text: "Third beat." }], persona);
    expect(s.turns[5]).toMatchObject({ role: "user", content: "All right, go on." });
  });

  it("is deterministic per pitch and persona", () => {
    expect(synthesizeSession("demo", segments, persona).id).toBe(synthesizeSession("demo", segments, persona).id);
  });
});
...
describe("buildSyntheticCalls", () => {
  ...
  it("redacts PII in persona scripts", () => {
    const leaky: SyntheticPersona = { id: "leaky", script: ["mail me at lead@example.com"], defaultReply: "ok" };
