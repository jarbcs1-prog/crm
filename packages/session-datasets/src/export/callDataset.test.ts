...
describe("definedScores", () => {
	it("keeps finite numbers and drops nulls", () => {
		expect(
			definedScores({
				id: "x",
				scores: { interest: 0.5, budget: null, urgency: Number.NaN },
			}).map((s) => s.key),
		).toEqual(["interest"]);
	});
});
...
describe("arenaCompare", () => {
	it("blinds pitch variants and crowns the higher mean", () => {
		...
		expect(arena.blinded.map((e) => e.label).sort()).toEqual([
			"Variant A",
			"Variant B",
		]);
...
describe("buildCallDataset", () => {
	...
	it("redacts PII without dropping the session", () => {
		const out = buildCallDataset([
			{ ...full, id: "pii", transcript: "reach me at rep@example.com please" },
		]);
