...
export function loadPersonas(path: string): SyntheticPersona[] {
	...
	for (const entry of list) {
		...
		const script = Array.isArray(rec.script)
			? rec.script.filter((s): s is string => typeof s === "string")
			: [];
		out.push({
			id: rec.id,
			script,
			defaultReply:
				typeof rec.defaultReply === "string" && rec.defaultReply
					? rec.defaultReply
					: "Go on.",
		});
	}
...
export function loadPitchSegments(dir: string): SyntheticPitchSegment[] {
	...
	for (const name of names) {
		...
		try {
			parsed = JSON.parse(
				readFileSync(join(dir, name), "utf-8") as string,
			) as unknown;
		} catch {
			continue;
		}
...
export function synthesizeSession(
	pitch: string,
	segments: SyntheticPitchSegment[],
	...
	for (let i = 0; i < pairs; i++) {
		...
		turns.push({
			role: "user",
			content: persona.script[i] ?? persona.defaultReply,
		});
	}
	turns.push({
		role: "assistant",
...
export function buildSyntheticCalls(
	...
	for (const [pitch, segs] of byPitch) {
		for (const persona of personas)
			all.push(synthesizeSession(pitch, segs, persona));
	}
...
if (isMain) {
	...
	if (!personasPath || !pitchesDir) {
		console.error(
			"Usage: bun run src/export/syntheticCalls.ts --personas <call-personas.json> --pitches <pitches dir> --out <canonical.jsonl>",
		);
		process.exit(1);
	}
	const outputs = buildSyntheticCalls(
		loadPersonas(personasPath),
		loadPitchSegments(pitchesDir),
	);
	writeFileSync(
		out,
	...
	console.log(
		`synthetic: ${outputs.sessions.length} sessions, ${outputs.duplicates} duplicates, ${outputs.redacted} redacted -> ${out}`,
	);
}
