...
import { type AgenticMiniRecord, type Session, toAgenticMini } from "../schema.js";

function ensureToolCallId(id: string | undefined, sessionId: string, idx: number): string {
  if (id) return id;
      ...
      if (m.tool_calls) {
        for (const tc of m.tool_calls) {
          ...
          if (!tc.function.arguments) tc.function.arguments = "{}";
            ...
            try { JSON.stringify(JSON.parse(tc.function.arguments)); } catch { tc.function.arguments = JSON.stringify(tc.function.arguments); }
          }
        }
      }
    }
...
export async function writeJsonl(records: AgenticMiniRecord[], outPath: string): Promise<void> {
  await mkdir(dirname(outPath), { recursive: true });
  const content = records.map((r) => JSON.stringify(r)).join("\n") + "\n";
  await writeFile(outPath, content, "utf-8");
}

export async function writeParquet(records: AgenticMiniRecord[], outPath: string): Promise<void> {
  await mkdir(dirname(outPath), { recursive: true });
  try {
  ...
  try {
    ...
    const res = spawnSync("python", [helper], { encoding: "utf-8", timeout: 30000 });
    if (res.stdout) process.stdout.write(res.stdout);
    if (res.stderr) process.stderr.write(res.stderr);
  } catch {}
}

export async function buildAgenticMini(inputPath: string, outPath: string): Promise<void> {
...
const isMain = process.argv[1]?.replace(/\\/g, "/").endsWith("adapters/agenticMini.ts") || process.argv[1]?.endsWith("agenticMini.js");
if (isMain) {
  const input = process.argv[2] ?? "data/canonical.jsonl";
  const output = process.argv[3] ?? "data/agentic-mini.jsonl";
  buildAgenticMini(input, output).then(() => console.log(`agentic-mini: ${input} -> ${output}`)).catch((e) => { console.error(e); process.exit(1); });
}
