...
const TAXONOMY_TASKS = ["debug_recovery", "command_execution", "tool_routing", "safety_boundary", "response_quality"] as const;
const SYSTEM_PROMPT = "You are Clawd, an AI assistant that routes tasks safely and executes tools responsibly.";

const HEURISTICS: Array<{ pattern: RegExp; task: string; taxonomy: string[] }> = [
  { pattern: /rm\s+-rf|sudo|chmod|delete|drop table/i, taxonomy: ["safety_boundary", "command_execution"], task: "safety_boundary" },
  { pattern: /error|fail|stack.?trace|exception/i, taxonomy: ["debug_recovery", "response_quality"], task: "debug_recovery" },
  { pattern: /exec|run|execute|command/i, taxonomy: ["command_execution"], task: "command_execution" },
  { pattern: /inappropriate|harmful|refuse|policy/i, taxonomy: ["safety_boundary"], task: "safety_boundary" },
];

function inferTaxonomy(session: Session): { taskType: string; taxonomy: string[] } {
  const text = session.turns.map((t) => t.content).join(" ");
  for (const h of HEURISTICS) {
    if (h.pattern.test(text)) return { taskType: h.task, taxonomy: h.taxonomy };
  }
  return { taskType: session.meta.task_type ?? "tool_routing", taxonomy: session.meta.tags.length ? session.meta.tags : ["tool_routing"] };
}
...
export function sessionToSafetyRouting(session: Session, idPrefix = "ds"): SafetyRoutingOutput {
  ...
  return {
    ...
    taxonomy,
    ...(session.meta.quality ? { quality: session.meta.quality as SafetyRoutingRecord["quality"] } : {}),
...
export async function buildSafetyRouting(inputPath: string, outPath: string): Promise<void> {
  ...
  for (const r of records) for (const t of r.taxonomy) dist[t] = (dist[t] ?? 0) + 1;
...
if (args.includes("--input")) {
  ...
  buildSafetyRouting(args[inputIdx + 1] as string, outIdx !== -1 ? args[outIdx + 1] as string : "data/safety-routing.jsonl").catch((e) => {
    console.error(e);
    process.exit(1);
  });
}
