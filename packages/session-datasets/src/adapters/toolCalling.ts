...
import {
	...
	type Session,
	type ToolCallingRecord,
	toToolCalling,
} from "../schema.js";

export function sessionToToolCallingRecords(
	session: Session,
	idPrefix = "tc",
): ToolCallingRecord[] {
	...
	return toolTurnIndices.map((idx, n) => {
		const userTurn = [...session.turns.slice(0, idx)]
			.reverse()
			.find((t) => t.role === "user");
		const assistantTurn = session.turns[idx]!;
		const toolResults = session.turns.filter(
			(t) =>
				t.role === "tool" &&
				assistantTurn.tool_calls?.some((tc) => tc.id === t.tool_call_id),
		);
		...
		messages.push({
			...
			tool_calls: (assistantTurn.tool_calls ?? []).map((tc) => ({
				...
				function: {
					name:
						canonicalizeToolName(tc.name) === "custom"
							? tc.name
							: canonicalizeToolName(tc.name),
		...
		return {
			...
			tags: [
				...session.meta.tags,
				...(assistantTurn.tool_calls ?? []).map((tc) =>
					canonicalizeToolName(tc.name),
				),
				assistantTurn.tool_calls && assistantTurn.tool_calls.length > 1
					? "multi_tool"
					: "single_tool",
			],
			messages,
...
export async function buildToolCalling(
	inputPath: string,
	outPath: string,
): Promise<void> {
...
if (args.length >= 2 && args[0] === "--input") {
	...
	buildToolCalling(
		args[inputIdx + 1]!,
		outIdx !== -1 ? args[outIdx + 1]! : "data/tool-calling.jsonl",
	).catch((e) => {
		console.error(e);
		process.exit(1);
	});
