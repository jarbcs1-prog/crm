...
const PATTERNS: Array<{ re: RegExp; replace: string }> = [
	...
	{
		re: /(?:api[_-]?key|apikey|secret|password|passwd|access_?token)\s*[:=]\s*["']?[^"'\s,}]+["']?/gi,
		replace: "<REDACTED>",
	},
];
...
export function scrubPii(text: string, opts: PiiOptions = {}): string {
	...
	for (const p of PATTERNS) {
		out = out.replace(
			p.re,
			p.replace.includes("<REDACTED>")
				? p.replace.replace("<REDACTED>", placeholder)
				: p.replace,
		);
	}
	return out;
...
export function scrubSession<
	T extends { turns: Array<{ content: string; reasoning_content?: string }> },
>(session: T, opts: PiiOptions = {}): { session: T; redacted: boolean } | null {
	let redacted = false;
