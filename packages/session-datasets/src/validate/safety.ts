...
async function main() {
	...
	if (!raw.trim()) {
		console.log(`safety: no data at ${input}`);
		process.exit(0);
	}
	...
	for (let i = 0; i < records.length; i++) {
		const r = records[i];
		if (!r!.id || !r!.task_type || !r!.messages || !r!.target) {
			console.error(`line ${i + 1}: missing required field`);
			errors++;
			continue;
		}
		...
		if (target.tool_calls && !Array.isArray(target.tool_calls)) {
			console.error(`line ${i + 1}: target.tool_calls not array`);
			errors++;
		}
	}
	console.log(
		`safety: ${records.length} records, ~${tokens} tokens, ${errors} errors`,
	);
