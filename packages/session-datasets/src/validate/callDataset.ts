...
async function main() {
	...
	for (let i = 0; i < records.length; i++) {
		...
		if (!CALL_SOURCES.has(String(meta.source ?? ""))) {
			console.error(
				`line ${i + 1}: unexpected source ${String(meta.source ?? "")}`,
			);
			errors++;
		}
	...
	if (judgeRaw.trim()) {
		const judged = JSON.parse(judgeRaw) as Array<{
			sessionId: string;
			score: number;
		}>;
		...
		if (judged.length !== records.length) {
			console.error(
				`judge: ${judged.length} results for ${records.length} sessions`,
			);
			errors++;
		}
	}
	...
	if (arenaRaw.trim()) {
		const arena = JSON.parse(arenaRaw) as {
			winner: string | null;
			blinded: Array<{ label: string }>;
		};
