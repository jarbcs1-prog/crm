export const TOOL_VOCAB = [
  "bash","read","edit","write","grep","glob","lsp","apply_patch","skill","todowrite","webfetch","websearch","task",
] as const;
...
export type SessionSource = "hermes-state-db" | "request-dump" | "opencode-agentic-mini" | "opencode-dataset" | "opencode-tool-calling" | "imported" | "call-transcript" | "synthetic-call";
...
export interface SessionMeta {
  ...
  chunk_index?: number;
  total_chunks?: number;
  quality?: { safety_ok?: boolean; tool_choice_ok?: boolean; command_validity_ok?: boolean };
...
export interface SafetyRoutingRecord {
  ...
  tools_available: string[];
  target: { assistant: string; tool_calls: Array<{ name: string; arguments: Record<string, unknown> }>; final_response: string };
  quality?: { safety_ok: boolean; tool_choice_ok: boolean; command_validity_ok: boolean };
...
export function canonicalizeToolName(name: string): CanonicalToolName {
  if ((TOOL_VOCAB as unknown as string[]).includes(name)) return name as CanonicalToolName;
  if (name in TOOL_ALIASES) return TOOL_ALIASES[name] as CanonicalToolName;
  return "custom";
}

export function toAgenticMini(session: Session, chunkIndex = 0, totalChunks = 1): AgenticMiniRecord {
  const ids = new Set(session.turns.filter((t) => t.tool_calls?.length).flatMap((t) => t.tool_calls!.map((c) => c.id)));
  const turns = session.turns.filter((t) => !(t.role === "tool" && !ids.has(t.tool_call_id ?? "")));
  return {
    messages: turns.map((t) => ({
      ...
      ...(t.name ? { name: t.name } : {}),
      ...(t.reasoning_content ? { reasoning_content: t.reasoning_content } : {}),
    })),
    metadata: {
...
export function toToolCalling(session: Session, idPrefix = "tc"): ToolCallingRecord {
  return {
        ...
        ...(t.tool_calls?.length
          ? {
              tool_calls: t.tool_calls.map((tc) => ({
                id: tc.id,
                type: "function" as const,
                function: { name: tc.canonicalName === "custom" ? tc.name : tc.canonicalName, arguments: tc.rawArguments },
              })),
            }
          : {}),
        ...(t.tool_call_id ? { tool_call_id: t.tool_call_id } : {}),
...
export function toSafetyRouting(session: Session, idPrefix = "ds"): SafetyRoutingRecord {
  ...
  return {
    ...
    messages: [
      { role: "system", content: session.meta.system_prompt ?? "You are Clawd, an AI assistant." },
