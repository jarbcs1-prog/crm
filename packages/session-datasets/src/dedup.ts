...
export function sessionHash(session: Session): string {
  ...
  for (const turn of session.turns) {
    ...
    if (turn.reasoning_content) parts.push(`reasoning:${normalizeContent(turn.reasoning_content)}`);
    if (turn.tool_calls?.length) {
      const sorted = [...turn.tool_calls].sort((a, b) => a.name.localeCompare(b.name) || a.id.localeCompare(b.id));
...
export function dedup<T extends Session>(sessions: T[]): { unique: T[]; duplicates: number } {
