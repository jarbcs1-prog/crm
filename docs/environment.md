...
| --- | --- |
| `TAVILY_API_KEY` | Web research with citations and the search that finds a LinkedIn slug |
| `EXA_API_KEY` | Semantic web search and research, similar to Perplexity |
| `BRAVE_API_KEY` | Privacy-focused web search results with content snippets |
| `FIRECRAWL_API_KEY` | Web scraping and content extraction from websites |
| `GOOGLE_API_KEY` / `GOOGLE_CSE_ID` | Traditional web search via Google Custom Search API |
...
Multiple local model providers are supported, all OpenAI-compatible endpoints. They allow running the agent without cloud egress.

| Variable | What it adds |
| --- | --- |
| `OLLAMA_BASE_URL` / `OLLAMA_MODEL` | Local Ollama server (e.g. `http://localhost:11434`) |
| `LMSTUDIO_BASE_URL` / `LMSTUDIO_MODEL` | Local LM Studio server |
...
| `OPENROUTER_MODEL` | Default model for OpenRouter calls |
