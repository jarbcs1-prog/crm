...
export default defineTool({
	...
	async execute({ callId }) {
		...
		if (isTerminalStatus(call.status)) {
			return { ok: false as const, alreadyEnded: true as const, status: call.status };
		}
		...
		await db.callEvent.create({
			data: { callId: call.id, type: CallEventType.HANGUP, payload: { hangupError } },
		});

		return { ok: true as const, ended: true as const, status: CallStatus.CANCELLED, hangupError, durationSecs };
	},
});
