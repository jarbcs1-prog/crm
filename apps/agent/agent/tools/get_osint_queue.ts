...
export default defineTool({
	...
	inputSchema: z.object({
		limit: z.number().int().min(1).max(50).optional().describe("How many targets to return. Defaults to 10."),
	}),
	async execute({ limit }) {
			...
			include: {
				contact: {
					select: {
						...
						phone: true,
						title: true,
						company: { select: { name: true, country: true, countryCode: true } },
					},
				},
			},
		});
		...
		const queue = targets.map((target) => ({
			...
			name: target.contact ? `${target.contact.firstName} ${target.contact.lastName ?? ""}`.trim() : null,
