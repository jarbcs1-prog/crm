...
function service(overrides: { findUnique?: unknown; transaction?: unknown }) {
		...
		deal: {
			...
			update: (args: unknown) => {
				...
				return Promise.resolve({
					id: "deal_1",
					stage: (args as { data: { stage: unknown } }).data.stage,
				});
			},
		},
		activity: {
...
describe("DealsService.setStage", () => {
	it("is a no-op when the stage is already the same", async () => {
		...
		const result = await svc.setStage(
			{ id: "deal_1", stage: DealStage.DEMO_BOOKED },
			"user_1",
		);
	...
	it("requires a reason when losing a deal", async () => {
		...
		expect(
			svc.setStage({ id: "deal_1", stage: DealStage.CLOSED_LOST }, "user_1"),
		).rejects.toThrow();
	});
	...
	it("logs a stage-change activity with from and to", async () => {
		...
		await svc.setStage(
			{ id: "deal_1", stage: DealStage.CONTRACT_SENT },
			"user_1",
		);
	...
	it("touches the activity stamp for the deal and company", async () => {
		...
		expect((seen.touched as { keys: unknown }).keys).toEqual({
			companyId: "co_1",
			dealId: "deal_1",
		});
	});
